...
export function useSensors() {
  ...
  useEffect(() => {
    ...
    const brokerUrl = isHttps ? "wss://broker.hivemq.com:8884/mqtt"
    ...
    client.onMessageArrived = async (msg: Message) => {
      ...
      try {
        ...
        const temp     = typeof raw.temperature === "number" ? raw.temperature
                         : typeof raw.temp === "number"       ? raw.temp
                         : undefined;

        const humidity = typeof raw.humidity === "number" ? raw.humidity : undefined;

        const gasRaw   = typeof raw.gas_raw === "number" ? raw.gas_raw
                         : typeof raw.gas === "number"    ? raw.gas
                         : undefined;

        const fireVal  = typeof raw.fire === "number"    ? raw.fire
                         : typeof raw.fire === "boolean"  ? (raw.fire ? 1 : 0)
                         : typeof raw.flame === "number"  ? raw.flame
                         : undefined;
        ...
        // 2) OPTIONAL: write to Firebase (safe + awaited)
        try {
          const { firebaseReady, getDatabaseSafe, getFirestoreSafe, DB_STRUCTURE } =
          ...
          await update(ref(rtdb), {
            [DB_STRUCTURE.REALTIME.SENSORS.GAS]:         { value: gasRaw ?? -1, timestamp: ts, status: getGasStatus(Math.round(gasRaw ?? 0)) },
            [DB_STRUCTURE.REALTIME.SENSORS.FIRE]:        { value: fireVal ?? 0, timestamp: ts, status: getFireStatus(fireVal ?? 0) },
            [DB_STRUCTURE.REALTIME.SENSORS.TEMPERATURE]: { value: temp ?? -1,  timestamp: ts, status: getTemperatureStatus(temp ?? -100) },
            [DB_STRUCTURE.REALTIME.SENSORS.HUMIDITY]:    { value: humidity ?? -1, timestamp: ts, status: "normal" },
            [DB_STRUCTURE.REALTIME.SENSORS.BUZZER]:      { value: (fireVal ?? 0) > 0, timestamp: ts, status: (fireVal ?? 0) > 0 ? "danger" : "normal" },
            [DB_STRUCTURE.REALTIME.SYSTEM.LAST_UPDATE]:  ts,
          });

          if (typeof gasRaw === "number") {
            await addDoc(collection(fs, DB_STRUCTURE.FIRESTORE.HISTORY.GAS), { value: Math.round(gasRaw), ts: serverTimestamp() });
          }
          if (typeof temp === "number") {
            await addDoc(collection(fs, DB_STRUCTURE.FIRESTORE.HISTORY.TEMPERATURE), { value: temp, ts: serverTimestamp() });
          }
          if (typeof fireVal === "number") {
            await addDoc(collection(fs, DB_STRUCTURE.FIRESTORE.HISTORY.FIRE), { value: fireVal, ts: serverTimestamp() });
          }
        } catch (fbErr) {
    ...
    client.connect({
      onSuccess: () => {
        ...
        // Use wide subscribe while debugging; switch to `${BASE_TOPIC}/telemetry` later
        client.subscribe("iot/firealarm/#");
        // client.subscribe(`${BASE_TOPIC}/telemetry`);
      },
      useSSL: isHttps, reconnect: true, keepAliveInterval: 30, timeout: 5,
    });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      off = onValue(sensorsRef, (snap) => setSensors((snap.val() || {}) as SensorMap));
    })();

    return () => { try { off && off(); } catch {} };
  }, []);

  // -------------------- NATIVE: danger notifications --------------------
  useEffect(() => {
    if (Platform.OS === "web") return;
    ...
    Object.entries(sensors).forEach(([key, val]) => {
      ...
      const gasStatus  = typeof s.gas === "number" ? getGasStatus(s.gas) : "normal";
      const tempStatus = typeof s.temperature === "number" ? getTemperatureStatus(s.temperature) : "normal";
      const fireStatus = typeof s.fire === "number" ? getFireStatus(s.fire) : "normal";

      const prev = prevStatusRef.current;

      if (gasStatus === "danger" && prev.gas !== "danger") {
        notifyDanger("Gas Alert", `High gas level on ${key}${s.gas != null ? ` (raw ${s.gas})` : ""}`);
      }
      if (tempStatus === "danger" && prev.temp !== "danger") {
        notifyDanger("Overheat Alert", `High temperature on ${key}${s.temperature != null ? ` (${s.temperature.toFixed(1)}°C)` : ""}`);
      }
